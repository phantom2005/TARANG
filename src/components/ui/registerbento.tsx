...
// NEW: Import reCAPTCHA component
import ReCAPTCHA from "react-google-recaptcha";
// NEW: Import GoogleOAuthProvider and GoogleLogin
...
// MODIFIED: AuthFormProps - Re-added onToggleForm for potential future use or if this component is reused
interface AuthFormProps {
  disableAnimations?: boolean; // Inherited from MagicBento
  enableTilt?: boolean;
  enableMagnetism?: boolean;
  enableBorderGlow?: boolean; // Inherited from MagicBento
  onToggleForm?: () => void; // Prop to switch forms if this component serves both
}
...
// Make sure this matches the key configured in your .env.local
...
// --- AuthForm Component (now a dedicated registration form with CAPTCHA and Google Login) ---
const AuthForm: React.FC<AuthFormProps> = ({
  disableAnimations,
  enableTilt,
  enableMagnetism,
  enableBorderGlow,
  onToggleForm, // Keeping this prop for flexibility
}) => {
  const title = "Start Your Musical Journey!";
  const buttonText = "Sign Up & Play";
  const toggleText = "Already a student?";
  const toggleLinkText = "Login Here";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log("Submitting registration with reCAPTCHA token:", recaptchaToken);
    // *** IMPORTANT: Send this data to your backend for actual registration and reCAPTCHA verification ***
    try {
        const response = await fetch('/api/register', { // Example backend endpoint
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({
                username: "Your Musician Name", // Replace with actual input value
                email: "test@example.com",     // Replace with actual input value
                password: password,
        ...
        recaptchaRef.current?.reset(); // Reset reCAPTCHA for next attempt
  ...
  const onRecaptchaChange = (token: string | null) => {
    ...
    if (token) {
      setRecaptchaError(''); // Clear error if token is received
    }
  };
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    // You MUST send this token to your backend for verification and user creation/login.
    try {
        const response = await fetch('/api/auth/google', { // Example backend endpoint
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        const data = await response.json();
        if (response.ok && data.success) {
            console.log("Backend verification successful, user registered/logged in:", data);
        ...
        console.error("Network error during Google login/registration verification:", error);
    }
  };
  ...
  return (
    <ParticleCard
      ...
      style={{
        width: "min(90%, 400px)",
        minHeight: "550px", // Increased height to accommodate reCAPTCHA and Google button
        position: "relative",
        zIndex: 10,
      }}
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-sm flex flex-col gap-4">
        <input
          type="text"
          placeholder="Your Musician Name"
        ...
        <input
          type="password"
          placeholder="Create Your Melody Password"
        ...
        <input
          type="password"
          placeholder="Confirm Melody Password"
          className="p-3 rounded-lg bg-[#1a0a2e] border border-[#523d6b] focus:outline-none focus:ring-2 focus:ring-[#8400ff] focus:border-transparent text-white placeholder-gray-400"
        ...
        {recaptchaError && (
          <p className="text-red-400 text-sm mt-1 text-center">{recaptchaError}</p>
        )}

        <button
          type="submit"
          className="bg-[#8400ff] text-white font-bold py-3 rounded-lg hover:bg-[#6a00cc] transition duration-200 ease-in-out mt-4"
        >
          {buttonText}
        </button>
      </form>
      ...
      <div className="w-full flex justify-center">
        <GoogleLogin
          ...
          theme="filled_blue"
          size="large"
          text="signup_with" // or "signin_with", "continue_with"
        />
      </div>

      <p className="mt-6 text-center text-gray-400">
        {toggleText}{" "}
        <a href="/login" className="text-[#8400ff] hover:underline font-medium">
          {toggleLinkText}
        </a>
      </p>
    </ParticleCard>
...
// --- SubscriptionCard Component (no changes needed) ---
  ...
  return (
    ...
    >
      ...
      <button className={`w-full py-3 rounded-lg font-semibold
        ...
        {name === "Virtuoso" ? "Contact Sales" : "Choose Plan"}
      </button>
    </ParticleCard>
  );
};
...
const MagicBento: React.FC<BentoProps> = ({
  ...
  // Removed showLoginForm state and handleToggleForm function
  // as AuthForm is now a dedicated signup form, linking to /login page directly.

  return (
    // NEW: Wrap the entire component in GoogleOAuthProvider
        ...
        <style>
          {`
            ...
            .card--border-glow::after {
              ...
              -webkit-mask: linear-gradient(#fff 0 0) content-box, linear-webkit-mask-composite: xor;
              pointer-events: none;
        ...
        >
          ...
          <div className="w-full max-w-5xl flex flex-col items-center justify-center gap-6 z-10 text-center">
            <h1 className="text-white text-5xl md:text-6xl font-extrabold leading-tight">
              Unleash Your Inner Musician with AI
            </h1>
            <p className="text-gray-300 text-lg md:text-xl max-w-3xl">
              Revolutionize your learning with AI-powered personalized lessons, instant feedback, and endless creative tools.
            </p>
            <div className="mt-8 flex flex-col sm:flex-row gap-4">
              <button
                className="px-8 py-4 bg-[#8400ff] text-white text-xl font-bold rounded-lg hover:bg-[#6a00cc] transition duration-300 ease-in-out shadow-lg"
              >
                Start Free Trial
              </button>
              <a
                href="/login" // Direct link to your login page
                className="px-8 py-4 bg-transparent border-2 border-[#8400ff] text-[#8400ff] text-xl font-bold rounded-lg hover:bg-[#8400ff] hover:text-white transition duration-300 ease-in-out shadow-lg flex items-center justify-center"
              >
          ...
          <div className="flex flex-col items-center justify-center gap-12 w-full max-w-6xl z-10 mt-12">
            {/* Auth Form (now only Signup with CAPTCHA and Google Sign-in) */}
            <div className="w-full flex justify-center">
              <AuthForm
                disableAnimations={shouldDisableAnimations}
                enableTilt={enableTilt}
                enableMagnetism={enableMagnetism}
                enableBorderGlow={enableBorderGlow}
                // No onToggleForm prop here as it links directly to /login
              />
            </div>
